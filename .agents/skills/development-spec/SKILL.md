...
- 当前项目采用“轻量版 Skill + 少量 Scripts”：Skill 负责流程、判断和质量标准，Scripts 只负责骨架创建、必要检查、API 文档生成和发布前检查。
...
如果使用 `props.ts` 的对象式定义，也要为每个属性写同样风格的 JSDoc 注释。
示例仅展示注释写法，具体字段名、类型和默认值以当前组件为准。
...
- 使用 `pnpm generate-api {ComponentName}` 生成 API 表格，直接读取 `props.ts` 里的中文 JSDoc 注释。
...
- 如果组件的 `props.ts` 缺少 JSDoc 注释，必须先补齐再生成文档。
- 不要把脚本扩展成复杂代码生成器；复杂组件的 API 设计、demo 设计和文档表达由 Skill 和实现者判断。
